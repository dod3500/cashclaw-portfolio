...
echo ""
echo -e "${CYAN}=========================================================${RESET}"
echo -e "  ${BOLD}Portable AI USB - Launcher (Linux)${RESET}"
echo -e "${CYAN}=========================================================${RESET}"
echo ""
...
setup_nvidia() {
    ...
    MODELS=$(curl -sf -H "Authorization: Bearer $USER_API_KEY" https://integrate.api.nvidia.com/v1/models 2>/dev/null | grep -oP '"id"\s*:\s*"[^"]*"' | sed 's/"id"\s*:\s*"//;s/"//' | head -20)
